...
export function earthTerrain(): Uint8Array {
  ...
  // убираем мелкие острова-«крошки» (< 100 клеток): при обзорном зуме это точки в
  // несколько пикселей, высаживаться на них смысла нет. Остаются только острова с
  // внятной площадью (≈10×10 клеток и крупнее). ВАЖНО: после прорезки каналов —
  // каналы могут отрезать от материка мелкие куски, их тоже убираем.
  const MIN_ISLAND = 100;
  // Белый список: острова меньше порога, которые всё равно оставляем (значимые/
  // узнаваемые). Точка [lon, lat] ≈ центр острова; компонент рядом с ней не режем.
  const KEEP: Array<[number, number]> = [
    [9.1, 40.1], // Сардиния (~69 клеток)
    [9.1, 42.2], // Корсика (~27)
    [121.0, 23.7], // Тайвань (~91)
    [109.7, 19.2], // Хайнань (~82)
  ];
  const keepPts = KEEP.map(([lon, lat]) => ({
    kx: ((lon + 180) / 360) * w,
    ky: ((LAT_TOP - lat) / LAT_SPAN) * h,
  }));
  const KEEP_R2 = 6 * 6; // радиус «узнавания» острова из белого списка (клетки)
  ...
  for (let c = 0; c < w * h; c++) {
    ...
    if (comp.length < MIN_ISLAND) {
      ...
      const protect = keepPts.some((p) => (p.kx - cx) ** 2 + (p.ky - cy) ** 2 <= KEEP_R2);
      if (!protect) for (const cc of comp) mask[cc] = 0;
    }
  }
