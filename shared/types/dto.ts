...
export interface WarshipPub {
  ...
  x: number;
  y: number;
  hp: number; // 0..1 — доля от АКТУАЛЬНОГО максимума (для полоски)
  // Попаданий по чужим боевым кораблям. Из него клиент сам считает и максимум
  // здоровья, и звание (warshipMaxHp/warshipRank), поэтому отдельных полей нет.
  // У новичков поле опущено — их большинство, а кадр состояния идёт каждый тик.
  xp?: number;
}
...
export interface BoatPub {
  ...
  // Секунд до прибытия: до берега высадки, а для отозванного десанта — до дома.
  // Считает сервер, потому что скорость хода — его параметр (BOAT_SPEED).
  eta: number;
  returning: boolean; // отозван и идёт назад
}
